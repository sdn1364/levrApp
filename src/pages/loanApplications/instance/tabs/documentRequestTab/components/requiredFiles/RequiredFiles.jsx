...
import { IconDownload, IconFileInfo, IconJpg, IconPlus, IconTrash } from "@tabler/icons";
...
const RequiredFiles = ({ docReq, parent }) => {
  ...
  return (isSuccess && allRequiredFiles.length > 0) && <Paper p="sm" sx={{ backgroundColor: theme.colorScheme === "light" ? theme.colors["gray"][0] : theme.colors["gray"][9] }}>
    ...
    <Stack mt="xl">
      {allRequiredFiles.map(({ name, file_extension }, index) => (
        <Group key={index} position="apart">
          ...
          <Group>
            <ActionIcon><IconDownload size={18} color={theme.colors["purple"][5]} /></ActionIcon>
