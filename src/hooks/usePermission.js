...
const usePermission = ({loanAppId,documentRequest, borrowerOrganizationId}) => {
  ...
  const canManageOrgRolesAndInvites = (organizationId) => {
    if(isSuccess){
      if (user.is_staff) return true;
      if (user.is_superuser) return true;
  ...
  const canManageDocRequestRolesAndInvites = ({documentRequest, borrowerOrganizationId}) => {
    if(isSuccess){
  ...
  const getAvailableRolesForOrganization = (organizationId) => {
    let id;
